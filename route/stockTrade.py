...
from models.dbStock import stockTrade
from route.auth import auth
import json

stockTradeApi = Blueprint('stockTradeApi', __name__)

cachePrefixGetAll = 'fundGetAll'


@stockTradeApi.route('/apis/stock/search')
@auth.login_required
def searchFund():
    # 按照关键字搜索股票
    # 获取股票历史行情
...
@stockTradeApi.route('/apis/stock/trade/add', methods=['POST'])
@auth.login_required
def addTradeRecord():
    # 添加股票交易记录
    rule = {
...
@stockTradeApi.route('/apis/stock/trade/getall')
@auth.login_required
...
@stockTradeApi.route('/apis/stock/trade/modify', methods=['PUT'])
@auth.login_required
...
@stockTradeApi.route('/apis/stock/trade/get')
@auth.login_required
def getById():
    # 按照id获取交易记录数据
    rule = {
...
@stockTradeApi.route('/apis/stock/trade/getbycode')
@auth.login_required
def getByCode():
    # 按照id获取交易记录数据
    rule = {
...
@stockTradeApi.route('/apis/stock/trade/del', methods=['DELETE'])
@auth.login_required
def delById():
    # 按照id 单条删除记录
    rule = {
...
@stockTradeApi.route('/apis/stock/import')
@auth.login_required
def importCsv():
    ...
    if(params['kind'] == 'trade'):
        fundsTradeDb = fundsTrade()
        fundsTradeDb.delAll()
        fo = open("./temp_data/股票交易记录完整版.csv", "r", encoding="utf-8")
        lines = fo.readlines()
        ...
        fo = open("./temp_data/股票持仓数据.csv", "r", encoding="utf-8")
        lines = fo.readlines()
